...
class Online_NN_section:
    ...
    def backward(self, error: np.ndarray, modify: bool = True, return_grads: bool = False,
                 learning_rate: float = 1e-3, return_error: bool = False) -> None:
        """
        ...
        for layer in reversed(self.model):
            ...
            grad = x['gradients']
            if grad is not None:
                grads = np.concatenate((grad, grads), axis=0)
    ...
    def validate_input(self, X: np.ndarray) -> None:
        ...
        """
        if X is None:
            raise ValueError('input data has not been provided')
    ...
    def Jaccobian(self) -> np.ndarray:
        ...
        jaccob = np.zeros((self.input.shape[0] * self.model[-1].output_size, self.trainable_params()))
        ...
        for out_ind in range(self.model[-1].output_size):
            ...
            jaccob[out_ind] = J_row

        return jaccob

    #################################################################

    def levenberg_mar(self, error_nn: np.ndarray,
    ...
    def EKF(self, error_nn: np.ndarray,
        ...
        if not hasattr(self, 'P'):
            self.P = P * np.eye(self.trainable_params()) if P is not None else 0.01 * np.eye(self.trainable_params())

        H = self.Jaccobian().T

        # Compute the normalization matrix (R + H^T * P * H)
        A = np.linalg.inv(R + H.T @ self.P @ H)

        # Compute the Kalman gain (K)
        K = self.P @ H @ A

        # Calculate gradient updates (new_grads)
        new_grads = K @ error_nn.T

        # Update the error covariance matrix (P)
        self.P -= K @ H.T @ self.P - Q
    ...
    def gauss_newton(self, error_nn: np.ndarray,
        ...
        **kwargs : dict
        ...
        try:
            ...
            new_grads = np.linalg.inv(J.T @ J + np.eye(self.trainable_params()) * 0.1) @ J.T @ error_nn.reshape((-1, 1))
